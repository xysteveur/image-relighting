...
def generate_mask(img, img_name, index, th=500, tw=300):
    ...
    cv2.imwrite(os.path.join(saveFolderMask,
...
saveFolder = 'result_both_crop_train_bg_blc_light2_chld5'
saveFolderMask = 'result_both_crop_train_bg_blc_light2_chld5/mask'
saveFolderLabel = 'result_both_crop_train_bg_blc_light2_chld5/label'
saveFolder = os.path.join(saveFolder, ARGS.model.split(".")[0])
if not os.path.exists(saveFolder):
    os.makedirs(saveFolder)
if not os.path.exists(saveFolderMask):
    os.makedirs(saveFolderMask)
if not os.path.exists(saveFolderLabel):
    os.makedirs(saveFolderLabel)
...
src_img, row, col, Lab, img_ori = preprocess_image('data/test/train_bg_blc/{}'.format(ARGS.source_image), 1)
...
label_path = "/home/steven/Projects/LBD/jiyao/lbid/full/labels/train_bg_blc/"
if crop_paste:
    for i in range(5):
        ...
        # print("image size")
        # print((row, col))
        # print((resultLab.shape[0],resultLab.shape[1]))
        assert(resultLab.shape[0]==row and resultLab.shape[1]==col)
        out_file = os.path.join(saveFolder,
                '{}_relit2_{}.jpg'.format(img_name, i))
        out_file_label = os.path.join(saveFolderLabel,
                '{}_relit2_{}.txt'.format(img_name, i))
        cv2.imwrite(out_file, resultTemp)
        if img_name+'.txt' not in os.listdir(label_path):
            raiseNameError("The image file doesn't have matched label file...")

        shutil.copyfile(os.path.join(label_path, img_name+'.txt'), out_file_label)
        print(img_name)
        print(out_file_label)
out_file_label_relit = os.path.join(saveFolderLabel,
        '{}_relit2.txt'.format(img_name)) 
cv2.imwrite(os.path.join(saveFolder,
        '{}_relit2.jpg'.format(img_name)), resultLab)
shutil.copyfile(os.path.join(label_path, img_name+'.txt'), out_file_label_relit)
print(out_file_label_relit)
print()
#----------------------------------------------